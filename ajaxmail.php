...
if(trim($un)!="" && trim($msg)!="" && trim($phone)!="" && trim($em)!=""){
		if(filter_var($em, FILTER_VALIDATE_EMAIL)){
			if(preg_match("/^[0-9]*$/", $phone) && strlen($phone)>6) {
				$message="Hi Admin..<p>".$un." has sent a query having Phone Number ".$phone." and email id as ".$em."</p><p>Query is : ".$msg."</p>";
				$headers = "MIME-Version: 1.0" . "\r\n";
				$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
				$headers .= 'From: <support@speedup.com>' . "\r\n";

				if(mail('support@templatebundle.net','Query for Travelio',$message,$headers )){
